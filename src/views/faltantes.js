...
export function renderFaltantes(root, ctx) {
  ...
  root.innerHTML = `
    <div class="layout">
      <section class="panel form-panel">
        ...
        <label>Proveedor (opcional)<input id="supplier" placeholder="Para agruparlo en pedidos"></label>
  ...
  function renderSuggestions() {
    $('#suggestions').innerHTML = ui.results.map(p => {
      const meta = [p.brand, p.unit, p.salePrice != null ? 'S/ ' + p.salePrice : '', p.stock != null ? 'stock ' + p.stock : '']
        .filter(Boolean).map(escapeHtml).join(' · ');
      return `<button class="suggestion" type="button" data-id="${escapeHtml(p.id)}">
  ...
  $('#save').addEventListener('click', async () => {
    try {
      await ctx.saveShortage({
        ...
        supplier: $('#supplier').value.trim() || primarySupplier(ui.selected),
      ...
      $('#search').value = ''; $('#qty').value = 1; $('#notes').value = ''; $('#unit').value = ''; $('#supplier').value = '';
